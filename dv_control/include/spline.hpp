...
// ============================================================
//  Projection result / open-path endpoint classification
// ============================================================

enum class SplineProjectionRegion
{
    ON_PATH_RANGE,
    BEFORE_START,
    AFTER_END
...
struct SplineProjectionResult
{
    bool valid = false;

    double s_proj = 0.0;
    Vec2 projected_point = Vec2(0.f, 0.f);
    double yaw_ref = 0.0;

    // Signed lateral error measured along the left normal:
    //   + : point lies to the left of the spline tangent
    //   - : point lies to the right of the spline tangent
    double ey = 0.0;

    // Signed displacement along the spline tangent at the projection:
    //   + : point lies ahead in the direction of increasing s
    //   - : point lies behind that direction
    double e_longitudinal = 0.0;

    // Closed paths are always ON_PATH_RANGE.
    SplineProjectionRegion region =
        SplineProjectionRegion::ON_PATH_RANGE;
};
...
public:
    void build(const std::vector<Vec2>& path, bool closed_loop)
    {
        ...
        open_start_segment_valid_ = false;
        open_start_point_ = Vec2(0.f, 0.f);
        open_start_tangent_x_ = 0.0;
        open_start_tangent_y_ = 0.0;
        ...
        {
            /*
                Store the direction of the first real raw path segment.

                BEFORE_START must be classified relative to the straight line
                through the first two distinct path points, not relative to a
                possibly different Akima endpoint tangent.
            */
            if (path.size() >= 2)
            {
                const Vec2 first = path.front();
                constexpr double kDuplicateEpsilonSquared = 1.0e-12;

                for (std::size_t i = 1; i < path.size(); ++i)
                {
                    const double dx =
                        static_cast<double>(path[i].x) -
                        static_cast<double>(first.x);

                    const double dy =
                        static_cast<double>(path[i].y) -
                        static_cast<double>(first.y);

                    const double length_squared =
                        dx * dx + dy * dy;

                    if (length_squared > kDuplicateEpsilonSquared)
                    {
                        const double length =
                        ...
                        open_start_tangent_x_ =
                            dx / length;

                        open_start_tangent_y_ =
                            dy / length;

                        open_start_segment_valid_ =
                            std::isfinite(open_start_tangent_x_) &&
                            std::isfinite(open_start_tangent_y_);
    ...
    // ------------------------------------------------------------
    // Projection with geometrically correct lateral error.
    //
    // ey is the projection of (q - p_proj) onto the LEFT normal,
    // not the full Euclidean distance to p_proj.
    //
    // For an open path the endpoint region is classified using the
    // longitudinal component along the endpoint tangent:
    //   s ~= 0 and e_longitudinal < 0 -> BEFORE_START
    //   s ~= L and e_longitudinal > 0 -> AFTER_END
    // ------------------------------------------------------------
    SplineProjectionResult projectPoint(
        const Vec2& q,
        double s_init,
        double search_radius = 3.0,
        int samples = 41,
        int newton_iters = 20) const
    {
        SplineProjectionResult result;

        if (!valid()) {
            return result;
        }

        const double L = totalLength();
        if (L <= 1.0e-12) {
            return result;
        }

        const double s_proj =
            projectToS(q, s_init, search_radius, samples, newton_iters);

        const Vec2 p = eval(s_proj);
        const double yaw = getYaw(s_proj);

        const double tx = std::cos(yaw);
        const double ty = std::sin(yaw);

        // Error vector from the projected spline point to q.
        const double ex = double(q.x) - double(p.x);
        const double ey_global = double(q.y) - double(p.y);

        // Left unit normal is n = [-ty, tx].
        // Therefore lateral error is n dot (q - p), equivalently cross(t, q-p).
        const double lateral_error =
            cross2(tx, ty, ex, ey_global);

        // Tangential component, used only to tell whether an open-path point
        // lies before the start or after the end.
        const double longitudinal_error =
            tx * ex + ty * ey_global;

        result.valid = true;
        result.s_proj = s_proj;
        result.projected_point = p;
        result.yaw_ref = yaw;
        result.ey = lateral_error;
        result.e_longitudinal = longitudinal_error;
        result.region = SplineProjectionRegion::ON_PATH_RANGE;
        ...
        constexpr double kLongitudinalEpsilon = 1.0e-6;

        if (s_proj <= s_epsilon &&
            longitudinal_error < -kLongitudinalEpsilon)
        {
            result.region = SplineProjectionRegion::BEFORE_START;
        }
        else if (s_proj >= L - s_epsilon &&
                 longitudinal_error > kLongitudinalEpsilon)
        {
            result.region = SplineProjectionRegion::AFTER_END;
        }
    ...
    // Existing calls remain valid; optional outputs expose endpoint handling.
    double signedNormalDistance(
        const Vec2& q,
        double s_init,
        double search_radius = 3.0,
        int samples = 41,
        int newton_iters = 20,
        double* s_proj_out = nullptr,
        Vec2* p_proj_out = nullptr,
        SplineProjectionRegion* region_out = nullptr,
        double* longitudinal_error_out = nullptr) const
    {
        const SplineProjectionResult projection =
            projectPoint(q, s_init, search_radius, samples, newton_iters);

        if (!projection.valid) {
            if (s_proj_out) *s_proj_out = 0.0;
            if (p_proj_out) *p_proj_out = Vec2(0.f, 0.f);
            if (region_out) {
                *region_out = SplineProjectionRegion::ON_PATH_RANGE;
            }
            if (longitudinal_error_out) {
                *longitudinal_error_out = 0.0;
            }
            return 0.0;
        }

        if (s_proj_out) *s_proj_out = projection.s_proj;
        if (p_proj_out) *p_proj_out = projection.projected_point;
        if (region_out) *region_out = projection.region;
        if (longitudinal_error_out) {
            *longitudinal_error_out = projection.e_longitudinal;
        }

        return projection.ey;
    }

    // Convenience: unsigned distance (absolute value)
    double normalDistance(const Vec2& q,
                          double s_init,
                          double search_radius = 3.0,
                          int samples = 41,
                          int newton_iters = 20,
                          double* s_proj_out = nullptr,
                          Vec2* p_proj_out = nullptr) const
    {
        const double d = signedNormalDistance(q, s_init, search_radius, samples, newton_iters, s_proj_out, p_proj_out);
        return std::abs(d);
    }

    
    double projectToSpline(const double x, const double y) const
    {
        return projectToSpline(Vec2((float)x, (float)y));
    }

    double projectToSpline(const Vec2& q) const
    {
        if (!valid()) return 0.0;
        ...
        // heurystyka: ~co 0.5 m, z clampem
        const double coarse_step = 0.5;
        int coarse_samples = (int)std::ceil(L / std::max(1e-6, coarse_step));
        coarse_samples = std::max(25, std::min(coarse_samples, 2000));

        double best_s  = 0.0;
        double best_d2 = std::numeric_limits<double>::infinity();

        if (closed_) {
            // closed: próbkuj [0, L) bez duplikatu końca
            for (int i = 0; i < coarse_samples; ++i) {
                const double s = (L * double(i)) / double(coarse_samples); // [0, L)
                const Vec2 p = eval(s);
            ...
            // open: próbkuj [0, L] z endpointem
            for (int i = 0; i < coarse_samples; ++i) {
                const double s = (L * double(i)) / double(coarse_samples - 1); // [0, L]
                const Vec2 p = eval(s);
        ...
        // refinement na Twojej istniejącej metodzie (domyślne argumenty)
        return projectToS(q, best_s);
    }


    // ------------------------------------------------------------
    // Stateless global projection used by path processing.
    //
    // BEFORE_START is reported only when:
    //   1. this is an open spline,
    //   2. the global position projection lands at s ~= 0,
    //   3. the bolide heading agrees with the first raw segment P0 -> P1,
    //   4. projection of (q - P0) onto P0 -> P1 is negative.
    //
    // AFTER_END is reported only when:
    //   1. this is an open spline,
    //   2. the global position projection lands at s ~= L,
    //   3. the bolide heading agrees with the endpoint spline tangent.
    //
    // Artificial straight projections are constructed later by path
    // processing. This function classifies the region and retains s on the
    // real spline range: 0 for BEFORE_START and L for AFTER_END.
    // ------------------------------------------------------------
    SplineProjectionResult projectPointGlobally(
        const Vec2& q,
        double vehicle_yaw) const
    {
        ...
        /*
            Closed splines have neither BEFORE_START nor AFTER_END.
        */
        if (closed_)
        {
            return result;
        ...
        const double vehicle_heading_x =
            std::cos(vehicle_yaw);

        const double vehicle_heading_y =
            std::sin(vehicle_yaw);

        /*
            BEFORE_START uses the first real raw segment, not the Akima
            derivative at s = 0.
        */
        const bool projection_is_at_start =
            s_proj >= -s_epsilon &&
            s_proj <= s_epsilon;

        if (projection_is_at_start &&
            open_start_segment_valid_)
        {
            const double start_to_q_x =
                static_cast<double>(q.x) -
                static_cast<double>(open_start_point_.x);

            const double start_to_q_y =
                static_cast<double>(q.y) -
                static_cast<double>(open_start_point_.y);

            const double longitudinal_from_start =
                start_to_q_x * open_start_tangent_x_ +
                start_to_q_y * open_start_tangent_y_;

            const double start_heading_alignment =
                vehicle_heading_x * open_start_tangent_x_ +
                vehicle_heading_y * open_start_tangent_y_;

            if (start_heading_alignment > 0.0 &&
                longitudinal_from_start < 0.0)
            {
                result.region =
                    SplineProjectionRegion::BEFORE_START;
                ...
                const double projection_x =
                    static_cast<double>(open_start_point_.x) +
                    longitudinal_from_start * open_start_tangent_x_;

                const double projection_y =
                    static_cast<double>(open_start_point_.y) +
                    longitudinal_from_start * open_start_tangent_y_;

                result.ey =
                    cross2(
                        open_start_tangent_x_,
                        open_start_tangent_y_,
                        static_cast<double>(q.x) - projection_x,
                        static_cast<double>(q.y) - projection_y
                    );
        ...
        const bool projection_is_at_end =
            s_proj >= L - s_epsilon &&
            s_proj <= L + s_epsilon;

        if (!projection_is_at_end)
        {
            return result;
        }

        /*
            AFTER_END uses the exact endpoint spline tangent.
        */
        const double yaw_end =
            getYaw(L);

        if (!std::isfinite(yaw_end))
        {
            return SplineProjectionResult{};
        }

        const double endpoint_tangent_x =
            std::cos(yaw_end);

        const double endpoint_tangent_y =
            std::sin(yaw_end);

        const double end_heading_alignment =
            vehicle_heading_x * endpoint_tangent_x +
            vehicle_heading_y * endpoint_tangent_y;

        if (end_heading_alignment > 0.0)
        {
            result.region =
                SplineProjectionRegion::AFTER_END;
...
private:
    ...
    bool open_start_segment_valid_ = false;
    Vec2 open_start_point_ = Vec2(0.f, 0.f);
    double open_start_tangent_x_ = 0.0;
    double open_start_tangent_y_ = 0.0;
