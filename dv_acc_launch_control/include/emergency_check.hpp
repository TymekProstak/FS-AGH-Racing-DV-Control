...
struct InsPoseCheck
{
    ...
    const double INS_STABLE_THRESHOLD = 2.0;
    ...
    )
    {
        ...
        if (!velocity_sliding_check_initialized)
        {
            velocity_sliding_check_initialized = true;
            prev_velocity_sliding_check_time_s = current_time_s;
            ins_velocity_sliding = false;
        ...
        const double time_window_s =
            current_time_s - prev_velocity_sliding_check_time_s;

        if (time_window_s >= INS_TIME_WINDOW_SLIDING)
        {
            const double abs_vx_other = std::abs(vx_body_other_mps);
            const double abs_vy_ins = std::abs(vy_body_ins_mps);

            speed_over_2_5mps =
                abs_vx_other > INS_SLIDING_MIN_SPEED_MPS;

            const bool lateral_velocity_big =
                abs_vy_ins >
                INS_SLIDING_LATERAL_TO_LONGITUDINAL_RATIO * abs_vx_other;

            const bool yaw_rate_small =
                std::abs(yaw_rate_rad_per_sec) <
                INS_SLIDING_MAX_YAW_RATE_RAD_PER_SEC;

            ins_velocity_sliding =
                speed_over_2_5mps &&
                lateral_velocity_big &&
                yaw_rate_small;

            prev_velocity_sliding_check_time_s = current_time_s;
        }

        hard_ins_velocity_sliding =
            UpdateHardHeldCondition(
                ins_velocity_sliding,
                hard_sliding_candidate_active,
                hard_sliding_candidate_start_time_s,
                current_time_s
            );

        return ins_velocity_sliding;
    }
    ...
    )
    {
        ...
        */
        ...
        else
        {
            ...
            if (use_ins_sliding_velocity_check)
            {
                CheckInsVelocitySliding(
                    curr_ins_velocity_body_xy_mps.x(),
                    curr_ins_velocity_body_xy_mps.y(),
                    vx_body_other_mps,
        ...
        is_emergency =
            ...
            (use_ins_stability_check && !ins_pose_is_stable) ||
            (use_ins_stability_check && !ins_velocity_is_stable) ||
            (use_ins_sliding_velocity_check && ins_velocity_sliding);

        is_hard_emergency =
            (use_ins_pose_check && hard_ins_stopped_sending) ||
            (use_ins_pose_check && !hard_ins_pose_is_changing) ||
            (use_ins_stability_check && !hard_ins_pose_is_stable) ||
            (use_ins_stability_check && !hard_ins_velocity_is_stable) ||
            (use_ins_sliding_velocity_check && hard_ins_velocity_sliding);
...
struct DynamicStateCheck
{
    ...
    bool ey_over_3m = false;
    bool epsi_over_90_deg = false;
    ...
    const double EPSI_THRESHOLD_RAD = 0.2617993877991494;
    ...
    )
    {
        ...
        beta_angle_over_20_deg =
            std::abs(curr_beta_rad) > BETA_THRESHOLD_RAD;

        yaw_rate_over_2_5_rad_per_sec =
            std::abs(curr_yaw_rate_rad_per_sec) >
            YAW_RATE_THRESHOLD_RAD_PER_SEC;
...
struct EmergencyCheck
{
    ...
    )
    {
        ...
        if (use_dynamic_state_check)
        {
            ...
            if (dynamic_state_check.ey_over_3m)
            {
                AppendEmergencyReason(
                    "Dynamic state lateral error too high, ey_m=" +
                    std::to_string(dynamic_state_check.curr_ey_m)
                );
            }

            if (dynamic_state_check.epsi_over_90_deg)
            {
                AppendEmergencyReason(
                    "Dynamic state heading error too high, epsi_rad=" +
                    std::to_string(dynamic_state_check.curr_epsi_rad)
                );
            }

            if (dynamic_state_check.beta_angle_over_20_deg)
            {
                AppendEmergencyReason(
                    "Dynamic state beta angle too high, beta_rad=" +
                    std::to_string(dynamic_state_check.curr_beta_rad)
                );
            }

            if (dynamic_state_check.yaw_rate_over_2_5_rad_per_sec)
            {
                AppendEmergencyReason(
                    "Dynamic state yaw rate too high, yaw_rate_rad_s=" +
                    std::to_string(dynamic_state_check.curr_yaw_rate_rad_per_sec)
                );
            }
        }
            ...
            use_ins_sliding_velocity_check)
        {
            ...
            if (use_ins_stability_check &&
                !ins_pose_check.ins_pose_is_stable)
            {
                AppendEmergencyReason(
                    "INS pose is unstable / jumping"
                );
            }

            if (use_ins_stability_check &&
                !ins_pose_check.hard_ins_pose_is_stable)
            {
                AppendHardEmergencyReason(
                    "HARD INS pose is unstable / jumping for long window"
                );
            }

            if (use_ins_stability_check &&
                !ins_pose_check.ins_velocity_is_stable)
            {
                AppendEmergencyReason(
                    "INS velocity is unstable, velocity_change_mps=" +
                    std::to_string(ins_pose_check.ins_velocity_change)
                );
            }

            if (use_ins_stability_check &&
                !ins_pose_check.hard_ins_velocity_is_stable)
            {
                AppendHardEmergencyReason(
                    "HARD INS velocity is unstable for long window"
                );
            }

            if (use_ins_sliding_velocity_check &&
                ins_pose_check.ins_velocity_sliding)
            {
                AppendEmergencyReason(
                    "INS velocity indicates sliding, vx_body_other_mps=" +
                    std::to_string(vx_body_other_mps) +
                    ", vx_body_ins_mps=" +
                    std::to_string(curr_ins_velocity_body_xy_mps.x()) +
                    ", vy_body_ins_mps=" +
                    std::to_string(curr_ins_velocity_body_xy_mps.y())
                );
            }

            if (use_ins_sliding_velocity_check &&
                ins_pose_check.hard_ins_velocity_sliding)
            {
                AppendHardEmergencyReason(
                    "HARD INS velocity indicates sliding for long window, vx_body_other_mps=" +
                    std::to_string(vx_body_other_mps) +
                    ", vx_body_ins_mps=" +
                    std::to_string(curr_ins_velocity_body_xy_mps.x()) +
                    ", vy_body_ins_mps=" +
